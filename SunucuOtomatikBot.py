...
def crashcontrol():
    ...
    while 1:
        ...
        if ((resim !=None) or (resim2 !=None)):
            ...
            bot.send_message(chatid,("%s botu durdu pandora programının tekrar başlatılmaya ihtiyacı var"% pcadi))
            break
        
    if kontrollu_kapatma==False:
            bot = telegram.Bot(apiid)
            
            bot.send_message(chatid,("%s botu hata aldı tekrar baslatılıyor"% pcadi))
            baslat()
            bot.send_message(chatid,("%s botu hata aldı tekrar başlatıldı"% pcadi))
...
def help(update, context):
    ...
    update.message.reply_text('start ve stop')
...
def error(update, context):
    ...
    bot.send_message(chatid,("%s botu hata aldı"% pcadi))
